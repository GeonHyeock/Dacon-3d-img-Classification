...
#model.load_state_dict(torch.load("/home/inhamath/dacon/GH_DACON_2022_08/model_state/40,0.0003.pt"))
...
def test(epoch):
    ...
    if epoch % 10 == 0:
        torch.save(model.state_dict(), path + f"/GH_DACON_2022_08/model_state/{epoch},{test_loss:.4f}.pt" )
    print(f'Test set: Average loss : {test_loss:.4f}, Accuracy : {correct}/{len(test_loader.dataset)}'
          f'({100. * correct / len(test_loader.dataset):.0f}%)') 
    return test_loss

since = time.time()
f = open(path + "/GH_DACON_2022_08/log2.txt", 'a')
