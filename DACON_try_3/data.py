...
def rotation(value,label):
    if int(label) in [2,5,6,9]:
        N = 1.5
    else:
        N = 3.14

    x,y,z = np.random.uniform(-N,N),np.random.uniform(-N,N),np.random.uniform(-N,N)
    rot_x = np.array([[
        [1,0,0],
        [0,np.cos(x),-np.sin(x)],
        [0,np.sin(x),np.cos(x)]
...
def data_scale(data):
    ...
    idx = np.linspace(0,data.shape[0]-1,1024).astype(int)
    return data[idx]
...
with open(path + '/GH_DACON_2022_08/train_dict.pickle', 'wb') as f:
    pickle.dump(train_dict, f)
"""

with open(path + "/GH_DACON_2022_08/train_dict.pickle", 'rb') as f:
...
for _ in range(9):
    train_dict["train_img"] += [rotation(value,label) for value,label in zip(pivot_data,pivot_label)]
    train_dict["train_label"] += pivot_label

dataset = GH_Dataset(np.array(train_dict["train_img"]),train_dict["train_label"])
train_dataset,test_dataset = torch.utils.data.random_split(dataset,[450000,50000])
